...
public class ObjectEntryValuesUtil {
	...
	public static Map<String, String> getAssigneeFieldValue(
		...
		Map<String, Long> assignTo = (Map<String, Long>)values.get("assignTo");
		...
		if (StringUtil.equals(className.getClassName(), Role.class.getName())) {
			Role role = RoleLocalServiceUtil.fetchRole(assignTo.get("classPK"));
		...
		User user = UserLocalServiceUtil.fetchUser(assignTo.get("classPK"));
