...
public class DocumentLibraryDDMFormFieldTemplateContextContributor
	...
	private User _createDDMFormGuestUser(long companyId) {
		try {
			...
			String screenName =
				DDMFormConstants.DDM_FORM_GUEST_USER_SCREEN_NAME;
	...
	private Folder _createPrivateUserFolder(
		...
		try {
			return dlAppService.addFolder(
				...
				String.valueOf(user.getScreenName()),
				LanguageUtil.get(
	...
	private User _getDDMFormGuestUser(long companyId) {
		try {
			...
			return _createDDMFormGuestUser(companyId);
		}
	}
	...
	private long _getFolderId(
		...
		try {
			...
			if (!themeDisplay.isSignedIn()) {
				user = _getDDMFormGuestUser(themeDisplay.getCompanyId());
			}
